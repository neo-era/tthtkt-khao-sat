...
- **Bảng chi tiết phải nằm khổ ngang**: mục IV và V nằm trong `<div class="ngang" data-kho="ngang">`. `catSection()` cắt báo cáo thành `div.WordSection1/2/3` theo **thuộc tính `data-kho`** (`"ngang"` mở section ngang, `"doc"` quay lại dọc) — **không** dò số La Mã nữa, cách cũ khiến thêm một mục là gãy hết bố cục khổ giấy mà không ai thấy. Bản in HTML dùng `@page ngang{size:A4 landscape}` + `.ngang{page:ngang}` nên PDF cũng ra đúng trang ngang như file Word.
- **Bảng nhiều cột PHẢI khai bề rộng cột** (`table-layout:fixed` + `<colgroup>` với `%` cộng đủ 100). Để trình duyệt tự chia thì `word-break` cho phép ngắt giữa chữ, nó bóp cột xuống còn **1 ký tự mỗi dòng** — bảng 16 cột từng in ra thành một rừng chữ cái xếp dọc. Đổi số cột thì phải sửa `<colgroup>` tương ứng.
